import { LogApp, LogLevel, RedisWorld, ServerPlayer, log } from '@shared';
import { User } from '@virtcon2/database-postgres';
import { ClientPacket, PacketType, RequestJoinPacketData, enqueuePacket } from '@virtcon2/network-packet';
...
export default async function request_join_packet(packet: ClientPacket<RequestJoinPacketData>, client: RedisClientType) {
  ...
  const initialServerPlayerState: ServerPlayer = {
    id: player.id,
